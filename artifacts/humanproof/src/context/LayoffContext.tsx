...
export interface LayoffState {
  ...
  alertDrift: { drift: number; direction: string; from: number; to: number; fromDate: string; daysSince: number } | null;
...
const layoffReducer = (state: LayoffState, action: LayoffAction): LayoffState => {
  switch (action.type) {
    case 'SET_INPUTS':
    ...
    case 'SET_SCORE_RESULT':
      return { ...state, scoreResult: action.payload, isCalculating: false, hasCompletedAssessment: true };
    case 'SET_HISTORY':
...
export const LayoffProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
